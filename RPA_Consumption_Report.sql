...
group by year(RCM.START_DATE),Month(RCM.START_DATE), RCM.COUNCIL_MEMBER_ID,RCM.NAME,RCM.COUNTRY,CM.COUNCIL_MEMBER_ID,year(RCM.END_DATE),Month(RCM.END_DATE);
...
from #Temp
group by Year(CREATE_DATE), Month(CREATE_DATE), REFERRER_CM_NAME, REFERRER_CM_ID, REFERRAL_CM_ID) a
where a.CREATE_YEAR>=2018
group by a.CREATE_YEAR, a.CREATE_MONTH, a.REFERRER_CM_NAME, a.REFERRER_CM_ID

--select *from #Active

select c.* into #TOTAL from
(select b.START_Year, b.START_Month, b.REFERRER_CM_NAME,
count(b.ranks) OVER (ORDER BY b.START_Year, b.START_Month rows 100000000 PRECEDING) total_partners from
(select a.* from
(select year(min(START_DATE)) AS START_Year, Month(min(START_DATE)) AS START_Month, REFERRER_CM_NAME, dense_rank() OVER (ORDER BY REFERRER_CM_ID) ranks
from #Temp
group by REFERRER_CM_NAME, REFERRER_CM_ID) a
group by a.START_Year, a.START_Month, a.REFERRER_CM_NAME, a.ranks) b
group by b.START_Year, b.START_Month, b.REFERRER_CM_NAME,b.ranks) c
...
group by Year(CREATE_DATE), Month(CREATE_DATE), REFERRER_CM_NAME, REFERRER_CM_ID,FIRST_GTC_DATE) a
...
from #Temp
where Year(CREATE_DATE)>=2020
group by REFERRER_CM_NAME, REFERRER_CM_ID,Year(CREATE_DATE), Month(CREATE_DATE)) a
group by a.REFERRER_CM_NAME, a.REFERRER_CM_ID, a.projects) b
...
select top 10* from WARS.BI.D_EMP
select top 10* from WARS.BI.D_COUNCIL_MEMBER_WORK_HISTORY
select top 10* from (SELECT CM.COUNCIL_MEMBER_ID
	, COUNT(CASE
	WHEN DP.PRODUCT_TYPE = 'PHONE CONSULTATION'
	THEN F_TPV_KEY
	END) AS CONSULT_TPV
...
FROM WARS.BI.F_TPV TPV
	...
	JOIN #REFERRERS R
	ON R.REFERRAL_CM_ID = CM.COUNCIL_MEMBER_ID
	GROUP BY CM.COUNCIL_MEMBER_ID) TPV*/
